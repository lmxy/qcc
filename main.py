...
for name in names:
    print(name)
    res = requests.get(url=urlEcode(name), headers=headers, cookies=get_cookies())
    # print(res.text)
    url_0 = getUlr(res)
    print('开始爬取第{}条数据'.format(i), url_0)
    # urls.append(url_0)
    print('waiting-------------------------15s')
    time.sleep(30) # 已验证15秒被封
    content = getContents_1(url_0)
    print(f'爬取了第{i}条数据', content)
    contents.append(content)
    print('waiting for more-------------------------10s')
    time.sleep(15)
    i += 1

df = pd.DataFrame.from_dict(contents)
print(df)
df.to_excel('企业基本信息.xlsx', index=False)
